...
PageJs.prototype = Object.extend(new BackEndPageJs(), {
	...
	,_getEditPanel: function(row) {
		...
		tmp.newDiv = new Element('tr', {'class': 'save-item-panel info'}).store('data', row)
			...
			.insert({'bottom': new Element('td', {'class': 'form-group'})
				.insert({'bottom': new Element('input', {'required': true, 'class': 'form-control', 'placeholder': 'The name of the Lanugage', 'save-item-panel': 'name', 'value': row.name ? row.name : ''}) })
			})
			.insert({'bottom': new Element('td', {'class': 'form-group'})
				.insert({'bottom': new Element('input', {'class': 'form-control', 'placeholder': 'The code of the Lanugage', 'save-item-panel': 'code', 'value': row.code ? row.code : ''}) })
			})
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': (tmp.isTitle === true ? 'success' : 'btn-hide-row')}).store('data', row)
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-1', 'style': tmp.isTitle ? 'font-weight:bold;' : ''}).update(row.name) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'name col-xs-1', 'style': tmp.isTitle ? 'font-weight:bold;' : ''}).update(row.code) })
